...
export default function LockpickingGame() {
  ...
  const [breakState, setBreakState] = useState<"none" | "bending" | "broken">(
    "none",
  );
  ...
  const breakTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
  const bendTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined,
  );
  const svgRef = useRef<SVGSVGElement>(null);

  useEffect(() => {
    sweetRef.current = sweetSpot;
  }, [sweetSpot]);
  ...
  const startTurning = useCallback(() => {
    ...
    const tick = (t: number) => {
      ...
      if (atSpot) {
        if (breakTimerRef.current !== undefined) {
          clearTimeout(breakTimerRef.current);
          breakTimerRef.current = undefined;
        }
        if (bendTimerRef.current !== undefined) {
          clearTimeout(bendTimerRef.current);
          bendTimerRef.current = undefined;
        }
        setBreakState("none");
        ...
        if (!bendTimerRef.current)
          bendTimerRef.current = setTimeout(
            () => setBreakState("bending"),
            BREAK_MS * 0.45,
          );
  ...
  const ringColor = !turning
    ...
    : d < SWEET_HALF
      ? "#eeab12" // gold  – at spot
      : d < SWEET_HALF * 2
        ? "#d4880c" // amber – very close
        : d < SWEET_HALF * 4
          ? "#c05020" // orange – moderate
          : d < SWEET_HALF * 7
            ? "#c02828" // red   – far
  ...
  return (
    <div
      className="bg-dark flex min-h-screen flex-col items-center justify-center gap-8 px-6 py-16"
    ...
    >
      ...
      <div className="text-center">
        ...
        <h1 className="text-cream text-3xl font-bold">Lockpicking</h1>
      </div>
        ...
        >
          ...
          {Array.from({ length: 24 }).map((_, i) => {
            ...
            return (
              <line
                key={i}
                x1={C + Math.cos(a) * (OUTER_R - 7)}
                y1={C + Math.sin(a) * (OUTER_R - 7)}
                x2={C + Math.cos(a) * (OUTER_R + 7)}
                y2={C + Math.sin(a) * (OUTER_R + 7)}
                stroke="#1e1b17"
                strokeWidth={1.5}
              />
            );
      ...
      <AnimatePresence mode="wait">
        {gameState === "playing" && (
          ...
          >
            <button
              ...
              className={`border px-10 py-4 text-xs tracking-widest uppercase transition-all duration-150 select-none ${
                turning
            ...
            <p className="text-muted/35 text-center text-xs leading-relaxed">
        ...
        {gameState === "won" && (
          ...
          >
            <p className="text-yellow text-2xl font-bold tracking-tight">
              Lock opened.
            </p>
            <button
              onClick={restart}
              className="border-muted/25 text-muted hover:border-muted/50 hover:text-cream border px-8 py-3 text-xs tracking-widest uppercase transition-all duration-150"
            >
              Try again
            </button>
          </motion.div>
        ...
        {gameState === "lost" && (
          <motion.div
            ...
            className="flex flex-col items-center gap-5"
          >
            <p className="text-pink text-2xl font-bold tracking-tight">
              Out of picks.
            </p>
            <button
              onClick={restart}
