...
interface LetterVar { r: number; s: number; }
interface PeekPos { x: number; y: number; rotate: number; fontSize: number; driftX: number; driftY: number; letters: LetterVar[]; }
...
const COLORS = ['#eeab12', '#e8437a', '#f5f0e8', '#ffd040', '#ff9040', '#eeab12'];
...
export default function NamePeek() {
  ...
  const [burst, setBurst] = useState<{ x: number; y: number; pieces: ConfettiPiece[] } | null>(null);
      ...
      <AnimatePresence>
        {peek && (
          <motion.div
            ...
            initial={{ opacity: 0, scale: 0.5, rotate: peek.rotate - 12, x: 0, y: 0 }}
            animate={{ opacity: 1, scale: 1, rotate: peek.rotate, x: peek.driftX, y: peek.driftY }}
          ...
          >
            <span
              className="text-muted/50 font-bold select-none flex overflow-hidden"
              style={{ fontSize: `${peek.fontSize}rem`, maxWidth: 'min(70vw, 520px)', padding: '0.3em 0.25em', margin: '-0.3em -0.25em' }}
            >
